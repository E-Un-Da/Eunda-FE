        ...
        <div style={{ 
          ...
          <Card >
            <Carousel ref={ carouselRef } {...settings}>
              {myStudies && myStudies.map((study) => (
                <a href="#" style={{ textDecoration: "none",  color: "black" }} onClick={ (e) => handlerOnClick(e, study.id) }>
                  <div key={study.id} style={{color: "black"}}>
                    <Meta
                      title={study.title}
                      description={study.intro}
                    />
                      <p style={{ marginTop: "8px", marginBottom: '8px' }}>Category: {study.category}</p>
                      <p style={{ marginBottom: '8px' }}>Leader: {study.leader}</p>
                      <p style={{ marginBottom: '8px' }}>Headcount: {study.headcount}</p>
                      <p style={{ marginBottom: '8px' }}>Recruit Num: {study.recruitNum}</p>
                      <p style={{ marginBottom: '8px' }}>Recruit: {study.recruit ? 'Yes' : 'No'}</p>
